...
def declare_logic():
    ...
    # Calculate the total fees due by summing the fees of all walks associated with the owner's dogs.
    Rule.sum(derive=Owner.total_fees_due, as_sum_of=Walk.fee, child_role_name='Dog')

    # Track the total number of walks a walker has conducted.
    Rule.count(derive=Walker.total_walks, as_count_of=Walk, where=None, child_role_name='Walker')

    # Calculate the walker’s total earnings as the sum of the fees earned from all walks.
    Rule.sum(derive=Walker.total_earnings, as_sum_of=Walk.fee, child_role_name='Walker')
